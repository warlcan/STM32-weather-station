...
static void NRF24_SPI_WriteReg(uint8_t reg, uint8_t val) {
...
void nrf24_init(void){
    ...
    NRF24_SPI_WriteReg(NRF24_REG_ADDR_EN_AA,      0x00);
    NRF24_SPI_WriteReg(NRF24_REG_ADDR_SETUP_RETR, 0x00);
    NRF24_SPI_WriteReg(NRF24_REG_ADDR_SETUP_AW,   0x03);
    NRF24_SPI_WriteReg(NRF24_REG_ADDR_RF_CH,      100);
    NRF24_SPI_WriteReg(NRF24_REG_ADDR_RF_SETUP,   0x07);

    uint8_t addr[5] = {0x9c, 0x96, 0xf1, 0x1f, 0x5e};
    nrf24_spi_transmit_buf(0x20 | 0x10, addr, 5);
    
    NRF24_SPI_WriteReg(NRF24_REG_ADDR_STATUS, 0x70);
}

bool nrf24_transmit_data(NRF24_Data_t *nrf24_data) {
    uint8_t nrf24_data_size = sizeof(NRF24_Data_t);

    NRF24_SPI_WriteReg(NRF24_REG_ADDR_CONFIG, 0x0E);
    ...
    NRF24_SPI_WriteReg(NRF24_REG_ADDR_STATUS, 0x20);
    NRF24_SPI_WriteReg(NRF24_REG_ADDR_CONFIG, 0x00);
    WAIT_FLAG(!LL_SPI_IsActiveFlag_BSY(NRF24_SPI), SPI_TIMEOUT);
    return true;
}